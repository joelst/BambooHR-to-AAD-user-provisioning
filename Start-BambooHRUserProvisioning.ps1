...
param (
  ...
  $MailboxDelegationParams = @(
    ...
    @{
      Group           = 'CG-SharedMailboxDelegatedAccessMailbox4'
      DelegateMailbox = 'Mailbox4'
    }
    @{
      Group           = 'CG-SharedMailboxDelegatedAccessMailbox5'
      DelegateMailbox = 'Mailbox3'
    }
  ),
  [parameter()]
...
function Write-PSLog {
  ...
  switch ($Severity) {
    ...
    Test {
      Write-Information " [TestOnly] $Message"
    }
    default {
      Write-Output $Message
    }
...
function Get-LicenseStatus {
  ...
  if ($licensesAvailable -lt 0 -and $NewUser.IsPresent) {
    Write-PSLog -Message "`n There are no licenses available for a newly created user!" -Severity Error
    $params = @{
      Message         = @{
        Subject      = 'BhrAadSync: There are no licenses available for a newly created user!'
        Body         = @{
          ContentType = 'html'
          Content     = "No licenses available for a newly created user. <br/> There are $($licensesConsumed) of $($licensesEnabled) assigned. $EmailSignature"
        }
        ToRecipients = @(
          @{
            EmailAddress = @{
  ...
  elseif ($licenses.ConsumedUnits -lt ($licensesEnabled - $MaximumExtraLicenses)) {
    Write-PSLog -Message "`n There are too many licenses left!" -Severity Error
    $params = @{
      Message         = @{
    ...
    Write-PSLog -Message "`n $($licensesConsumed) of $($licensesEnabled) licenses $LicenseId have been assigned." -Severity Information
  }
...
#>
  ...
  if ($Permissions -contains 'FullAccess') {
    ...
    $existingFullAccessPermissions = Get-EXOMailboxPermission -Identity $mObj.identity |
      Sort-Object -Property User -Unique | Where-Object { $_.User -notlike '*SELF' } |
      Sort-Object -Unique -Property User | ForEach-Object { Get-MgUser -UserId $_.User }
    if ($gMembers) {
  ...
  if ($Permissions -contains 'SendAs') {
    ...
    $existingSendAsPermissions = Get-EXORecipientPermission -Identity $mObj.identity |
      Where-Object { $_.Trustee -like '*@*' -and $_.AccessControlType -eq 'Allow' -and $_.AccessRights -contains 'SendAs' } |
      Sort-Object -Property Trustee -Unique | ForEach-Object { Get-MgUser -UserId $_.Trustee }
    if ($gMembers) {
...
Write-PSLog -Message "Executing Connect-MgGraph -TenantId $TenantID" -Severity Debug
...
if ([string]::IsNullOrWhiteSpace($testUser)) {
  Write-PSLog -Message "Unable to obtain user information using Get-MgUser -UserId $AdminEmailAddress" -Severity Error
  exit 1
}
...
# Check if variables are not set. If there is an environment variable, set its value to the variable.
#Used as an Azure Function
if ([string]::IsNullOrWhiteSpace($BambooHrApiKey) -and [string]::IsNullOrWhiteSpace($env:BambooHrApiKey)) {
  Write-PSLog 'BambooHR API Key not defined' -Severity Error
  New-AdaptiveCard {
...
elseif ([string]::IsNullOrWhiteSpace($AdminEmailAddress) -and
  (-not [string]::IsNullOrWhiteSpace($env:BambooHrApiKey))) {
  $BambooHrApiKey = $env:BambooHrApiKey
}

if ([string]::IsNullOrWhiteSpace($AdminEmailAddress) -and [string]::IsNullOrWhiteSpace($env:AdminEmailAddress)) {
  Write-PSLog 'Admin Email Address not defined' -Severity Error
  New-AdaptiveCard {
...
elseif ([string]::IsNullOrWhiteSpace($AdminEmailAddress) -and
  (-not [string]::IsNullOrWhiteSpace($env:AdminEmailAddress))) {
...
if ([string]::IsNullOrWhiteSpace($BHRCompanyName) -and [string]::IsNullOrWhiteSpace($env:BHRCompanyName)) {
  Write-PSLog 'BambooHR company name not defined' -Severity Error
  New-AdaptiveCard {
...
elseif ([string]::IsNullOrWhiteSpace($BHRCompanyName) -and
  (-not [string]::IsNullOrWhiteSpace($env:BHRCompanyName))) {
  $BHRCompanyName = $env:BHRCompanyName
}

if ([string]::IsNullOrWhiteSpace($CompanyName) -and [string]::IsNullOrWhiteSpace($env:CompanyName)) {
  Write-PSLog 'Company name not defined' -Severity Error
  New-AdaptiveCard {
...
if ([string]::IsNullOrWhiteSpace($TenantID) -and [string]::IsNullOrWhiteSpace($env:TenantID)) {
  Write-PSLog 'TenantId not defined' -Severity Error
  New-AdaptiveCard {
...
Write-PSLog -Message "Starting BambooHR to Entra AD synchronization at $(Get-Date)" -Severity Information
...
try {
  ...
  # write to log file and exit script

  Write-PSLog -Message "Error calling BambooHr API for user information. `nEXCEPTION MESSAGE: $($_.Exception.Message) `n CATEGORY: $($_.CategoryInfo.Category) `n SCRIPT STACK TRACE: $($_.ScriptStackTrace)" -Severity Error
  ...
  #Send-TeamsCard -CardSubject "BhrAadSync error: BambooHR connection failed"  -Message
  # "BambooHR connection failed. <br/> EXCEPTION MESSAGE: $($_.Exception.Message) <br/>
  # CATEGORY: $($_.CategoryInfo.Category)<br/> SCRIPT STACK TRACE: $($_.ScriptStackTrace) `n $EmailSignature"
  exit
}

# If no error returned, it means that the script was not interrupted by the "Exit" command within the "Catch".
# Write info below to log file and continue
Write-PSLog -Message 'Successfully extracted employee information from BambooHR.' -Severity Debug
...
if ($?) {
  ...
  Write-PSLog -Message "Successfully connected to AAD: $TenantId." -Severity Debug
}
else {
...
Write-PSLog -Message "Removing employee records that do not have a company email address of $companyEmailDomain" -Severity Debug

# Only select employees with a company email.
$employees | Sort-Object -Property LastName |
  Where-Object { $_.workEmail -like "*$companyEmailDomain" } | ForEach-Object {
    $error.Clear()
    # On each loop, pass all employee data from BambooHR to variables, to be compared one by one with the user data
    # from Azure AD and set them, if necessary
  ...
  #>
    ...
    if (([datetime]$bhrHireDate) -le (Get-Date).AddDays($DaysAhead)) {
      ...
      Get-MgUser -UserId $bhrWorkEmail -Property id, userprincipalname, Department, EmployeeId, JobTitle, CompanyName, Surname, GivenName, DisplayName, AccountEnabled, Mail, EmployeeHireDate, OfficeLocation, BusinessPhones, MobilePhone, OnPremisesExtensionAttributes, AdditionalProperties -ExpandProperty manager -ErrorAction SilentlyContinue -OutVariable aadUpnObjDetails | Out-Null
      ...
      if ([string]::IsNullOrEmpty($aadUpnObjDetails) -eq $false) {
        $UpnExtensionAttribute1 = ($aadUpnObjDetails |
            Select-Object @{
        ...
        if ([string]::IsNullOrEmpty($aadEidObjDetails) -eq $false) {
          $EIDExtensionAttribute1 = ($aadEidObjDetails |
              Select-Object @{
                Name       = 'ExtensionAttribute1'
                Expression = { $_.OnPremisesExtensionAttributes.ExtensionAttribute1 }
              } -ErrorAction SilentlyContinue).ExtensionAttribute1
          ...
          $aadSupervisorEmail = "$(($aadUpnObjDetails |
          ...
          Write-PSLog -Message "AAD Upn Obj Details: '$([string]::IsNullOrEmpty($aadUpnObjDetails) -eq $false)' AadEidObjDetails: $([string]::IsNullOrEmpty($aadEidObjDetails) -eq $false) = $(([string]::IsNullOrEmpty($aadUpnObjDetails) -eq $false) -or ([string]::IsNullOrEmpty($aadEidObjDetails) -eq $false))" -Severity Debug

          if (([string]::IsNullOrEmpty($aadUpnObjDetails) -eq $false) -or ([string]::IsNullOrEmpty($aadEidObjDetails) -eq $false)) {
            Write-PSLog -Message "Entra Id user: $aadFirstName $aadLastName ($aadDisplayName) $aadWorkEmail" -Severity Debug
            Write-PSLog -Message "Department: $aadDepartment, Title: $aadJobTitle, Manager: $aadSupervisorEmail, HireDate: $aadHireDate" -Severity Debug
            Write-PSLog -Message "EmployeeId: $aadEmployeeNumber, Enabled: $aadStatus OfficeLocation: $aadOfficeLocation, WorkPhone: $aadWorkPhone" -Severity Debug

            <# If empID of object returned by UPN or by empID is equal, and if object ID is the same as
            object ID of object returned by UPN and EmpID and if UPN = workemail from bamboo AND if the
            last changed date from BambooHR is NOT equal to the last changed date saved in
            ExtensionAttribute1 in AAD, check each attribute and set them correctly, according to BambooHR
            #>

            Write-PSLog -Message "Entra Id Employee Number: $aadEmployeeNumber -eq $aadEmployeeNumber2 = $($aadEmployeeNumber -eq $aadEmployeeNumber2) -and `
            ...
            if (($aadEmployeeNumber -ne $bhrEmployeeNumber) -and ($aadUpnObjDetails.UserPrincipalName -eq $bhrWorkEmail) -and `
              ...
              Write-PSLog -Message "Entra Id Employee number $aadEmployeeNumber does not match BambooHR $bhrEmployeeNumber, but the UPN matches. Update the employee number in AAD." -Severity Debug
              $error.clear()
              if ($TestOnly.IsPresent) {
                Write-PSLog -Message "Executing: Update-MgUser -UserId $bhrWorkEmail -EmployeeId $bhremployeeNumber" -Severity Test
              }
              else {
            ...
            if ($aadEmployeeNumber -eq $aadEmployeeNumber2 -or
              ...
              if ($bhrAccountEnabled -eq $false -and $bhrEmploymentStatus.Trim() -eq 'Terminated' -and $aadStatus -eq $true ) {
                ...
                $error.clear()
                if ($TestOnly.IsPresent) {
                  Write-PSLog -Message "Executing: Update-MgUser -UserId $bhrWorkEmail -AccountEnabled:$bhrAccountEnabled" -Severity Test
                  Write-PSLog -Message "Executing: Revoke-MgUserSignInSession -UserId $bhrWorkEmail" -Severity Test
                }
                else {
                ...
                if ($TestOnly.IsPresent) {
                  Write-PSLog -Message "Executing: Update-MgUser -UserId $bhrWorkEmail -BodyParameter $params" -Severity Test
                  Write-PSLog -Message "Executing: Update-MgUser -UserId $bhrWorkEmail -Department 'Not Active' -JobTitle 'Not Active $(Get-Date)' -OfficeLocation 'Not Active' -BusinessPhones '0' -MobilePhone '0' -CompanyName 'Not Active'" -Severity Test
                  Write-PSLog -Message "Executing: Get-MgUserMemberOf -UserId $bhrWorkEmail" -Severity Test
                  Write-PSLog -Message "Executing: $null = Update-MgUser -UserId $bhrWorkEmail -OnPremisesExtensionAttributes @{extensionAttribute1 = '$bhrLastChanged' }" -Severity Test

                  Write-PSLog -Message "Converting $bhrWorkEmail to a shared mailbox..." -Severity Test
                  Write-PSLog -Message "Executing: Set-Mailbox -Identity $bhrWorkEmail -Type Shared" -Severity Test
                  # Give permissions to converted mailbox to previous manager
                  Write-PSLog "Executing: Add-MailboxPermission -Identity $($mObj.Id) -User $aadSupervisorEmail -AccessRights ‘FullAccess’ -Automapping:$true –inheritancetype All" -Severity Test

                  Write-PSLog -Message 'Removing licenses...' -Severity Test
                  Write-PSLog -Message "Executing: Get-MgUserLicenseDetail -UserId $bhrWorkEmail | ForEach-Object { Set-MgUserLicense -UserId $bhrWorkEmail -RemoveLicenses $_.SkuId -AddLicenses @{} }" -Severity Test

                  Write-PSLog -Message 'Remove group memberships' -Severity Test
                  Write-PSLog -Message "Executing: Get-MgUserMemberOf -UserId $bhrWorkEmail | ForEach-Object { Remove-MgGroupMemberByRef -GroupId $_.id -DirectoryObjectId $aadUpnObjDetails.id } " -Severity Test

                  Write-PSLog -Message 'Remove MFA auth for user' -Severity Test
                }
                else {
                  ...
                  Write-PSLog "Executing: Add-MailboxPermission -Identity $($mObj.Id) -User $aadSupervisorEmail -AccessRights ‘FullAccess’ -Automapping:$true –inheritancetype All" -Severity Debug
                  ...
                  Write-PSLog -Message "Executing: Get-MgUserMemberOf -UserId $bhrWorkEmail | ForEach-Object { Remove-MgGroupMemberByRef -GroupId $_.id -DirectoryObjectId $aadUpnObjDetails.id } " -Severity Debug
              ...
              elseif ($bhrAccountEnabled -eq $false -and $bhrEmploymentStatus.Trim() -eq 'Terminated' -and $aadStatus -eq $false ) {
                ...
                if ($bhrAccountEnabled -eq $true -and $aadstatus -eq $false) {
                  ...
                  if ($TestOnly.IsPresent) {
                    ...
                    Write-PSLog -Message "Executing: Update-MgUser -UserId $bhrWorkEmail -AccountEnabled:$bhrAccountEnabled" -Severity Test
                    Write-PSLog -Message "Executing: Update-MgUser -UserId $bhrWorkEmail -BodyParameter $params" -Severity Test
                    Write-PSLog -Message "Executing: Set-Mailbox -Identity $bhrWorkEmail -Type Regular" -Severity Test
                ...
                if ($aadJobTitle.Trim() -ne $bhrJobTitle.Trim()) {
                  Write-PSLog -Message "AAD Job Title $aadJobTitle does not match BHR Job Title $bhrJobTitle" -Severity Debug

                  if ($TestOnly.IsPresent) {

                    Write-PSLog -Message " Executing: Update-MgUser -UserId $bhrWorkEmail -JobTitle $bhrJobTitle" -Severity Test
                  }
                  else {
                    ...
                    if (!$?) {
                      ...
                      $error.Clear()
                      Write-PSLog -Message "JobTitle for $bhrWorkEmail in AAD set from '$aadjobTitle' to '$bhrjobTitle'." -Severity Information
                    }
                  }
                }
                ...
                if ($aadDepartment.Trim() -ne $bhrDepartment.Trim()) {
                  Write-PSLog -Message "AAD department '$aadDepartment' does not match BambooHR department '$($bhrDepartment.Trim())'" -Severity Debug
                  if ($TestOnly.IsPresent) {
                    Write-PSLog -Message "Executing: Update-MgUser -UserId $bhrWorkEmail -Department $bhrDepartment" -Severity Test
                  }
                  else {
                    ...
                    if (!$?) {
                      ...
                      $error.Clear()
                      Write-PSLog -Message "Department for $bhrWorkEmail in AAD set from '$aadDepartment' to '$bhrDepartment'." -Severity Information
                    }
                  }
                }
                else {
                  Write-PSLog "AAD and BHR department already matches $aadDepartment" -Severity Debug
                }
                ...
                if ($aadSupervisorEmail -ne $bhrSupervisorEmail -and ([string]::IsNullOrWhiteSpace($bhrSupervisorEmail) -eq $false)) {
                  Write-PSLog -Message "Manager in AAD '$aadSupervisorEmail' does not match BHR manager '$bhrSupervisorEmail'" -Severity Debug
                  ...
                  if ($TestOnly.IsPresent) {
                    Write-PSLog -Message "Executing: Set-MgUserManagerByRef -UserId $bhrWorkEmail -BodyParameter $newManager"
                  }
                  else {
                    ...
                    if (!$?) {
                      ...
                      $error.Clear()
                      Write-PSLog -Message "Manager of $bhrWorkEmail in AAD '$aadsupervisorEmail' and in BambooHR '$bhrsupervisorEmail'. Setting new manager to the Azure User Object." -Severity Information
                    }
                  }
                }
                else {
                  Write-PSLog -Message "Supervisor email already correct $aadSuperVisorEmail" -Severity Debug
                }

                # Check and set the Office Location
                if ($aadOfficeLocation.Trim() -ne $bhrOfficeLocation.Trim()) {
                  Write-PSLog -Message "AAD office location '$aadOfficeLocation' does not match BHR hire data '$bhrOfficeLocation'" -Severity Debug
                  if ($TestOnly.IsPresent) {

                    Write-PSLog -Message "Executing: Update-MgUser -UserId $bhrWorkEmail -OfficeLocation $($bhrOfficeLocation.Trim())" -Severity Test
                  }
                  else {
                    ...
                    if (!$?) {
                      ...
                      $error.Clear()
                      Write-PSLog -Message "Office location of $bhrWorkEmail in AAD changed from '$aadOfficeLocation' to '$bhrOfficeLocation'." -Severity Information
                    }
                  }
                }
                else {
                  Write-PSLog -Message "Office Location correct $aadOfficeLocation" -Severity Debug
                }

                # Check and set the Employee Hire Date
                if ($aadHireDate -ne $bhrHireDate) {
                  Write-PSLog -Message "AAD hire date '$aadHireDate' does not match BHR hire data '$bhrHireDate'" -Severity Debug
                  if ($TestOnly.IsPresent) {

                    Write-PSLog -Message "Executing: Update-MgUser -UserId $bhrWorkEmail -EmployeeHireDate $bhrHireDate" -Severity Test
                  }
                  else {
                    ...
                    if (!$?) {
                      ...
                      $error.Clear()
                      Write-PSLog -Message "Hire date of $bhrWorkEmail changed from '$aadHireDate' in AAD and BHR '$bhrHireDate'." -Severity Information
                    }
                  }
                }
                else {
                ...
                if (($aadWorkPhone) -ne ($bhrWorkPhone)) {
                  ...
                  Write-PSLog -Message "AAD work phone '$aadWorkPhone' does not match BHR '$bhrWorkPhone'" -Severity Debug
                  ...
                  if ($TestOnly.IsPresent) {
                    ...
                    Write-PSLog -Message "Executing: Update-MgUser -UserId $bhrWorkEmail -BusinessPhones $bhrWorkPhone" -Severity Test
                  }
                  else {
                    ...
                    if (!$?) {
                      ...
                      $error.Clear()
                      Write-PSLog -Message "Work Phone for '$bhrWorkEmail' changed from '$aadWorkPhone' to '$bhrWorkPhone'" -Severity Information
                    }
                  }
                }
                else {
                  Write-PSLog -Message "Work phone correct $aadWorkEmail $aadWorkPhone" -Severity Debug
                }
                ...
                if ($EnableMobilePhoneSync.IsPresent) {
                  ...
                  if ($aadMobilePhone -ne $bhrMobilePhone) {
                    ...
                    Write-PSLog -Message "AAD mobile phone '$aadWorkPhone' does not match BHR '$bhrMobilePhone'" -Severity Debug

                    if ($TestOnly.IsPresent) {

                      Write-PSLog -Message "Executing: Update-MgUser -UserId $bhrWorkEmail -MobilePhone $bhrMobilePhone" -Severity Test
                    }
                    else {
                      ...
                      if (!$?) {
                        ...
                        $error.Clear()
                        Write-PSLog -Message "Work Mobile Phone for '$bhrWorkEmail' changed from '$aadMobilePhone' to '$bhrMobilePhone'" -Severity Deb
                      }
                    }
                  }
                  else {
                    Write-PSLog -Message "Mobile phone correct for $aadWorkEmail $aadMobilePhone" -Severity Debug
                  }
                }

                # Compare user employee id with BambooHR and set it if not correct
                if ($bhrEmployeeNumber.Trim() -ne $aadEmployeeNumber.Trim()) {
                  Write-PSLog -Message " BHR employee number $bhrEmployeeNumber does not match AAD employee id $aadEmployeeNumber" -Severity Debug
                  if ($TestOnly.IsPresent) {
                  ...
                  Write-PSLog -Message "Employee ID matched $bhrEmployeeNumber and $aadEmployeeNumber" -Severity Debug
                }

                # Set Company name to $CompanyName"
                if ($aadCompanyName.Trim() -ne $CompanyName.Trim()) {
                  Write-PSLog -Message "AAD company name '$aadCompany' does not match '$CompanyName'" -Severity Debug
                  if ($TestOnly.IsPresent) {

                    Write-PSLog -Message "Executing: Update-MgUser -UserId $bhrWorkEmail -CompanyName $($CompanyName.Trim())" -Severity Information -Severity Test
                  }
                  else {
                    ...
                    if (!$?) {
                      ...
                      Write-PSLog -Message " The $bhrWorkEmail employee Company attribute has been set to: $CompanyName." -Severity Information
                    }
                  }
                }
                else {
                  Write-PSLog -Message "Company name already matched in AAD and BHR $aadCompanyName" -Severity Debug
                }
          ...
          if (($aadEmployeeNumber2 -eq $bhremployeeNumber) -and ($historystatus -notlike '*inactive*') -and ($aadUpnObjDetails.id -eq $aadEidObjDetails.id)) {
            ...
            Write-PSLog -Message "Evaluating if AAD name change is required for $aadfirstName $aadlastName ($aaddisplayname) `n`t Work Email: $aadWorkEmail UserPrincipalName: $aadUpn EmployeeId: $aadEmployeeNumber" -Severity Debug
            ...
            if ($EIDExtensionAttribute1 -ne $bhrlastChanged) {
              if ($TestOnly.IsPresent) {
                Write-PSLog -Message "The $bhrWorkEmail employee LastChanged attribute set to extensionAttribute1 as $bhrlastChanged." -Severity Test
                Write-PSLog -Message "Executing: Update-MgUser -UserId $aadObjectID -OnPremisesExtensionAttributes @{extensionAttribute1 = $bhrlastChanged } " -Severity Test
              }
              else {
                # Setting the "lastchanged" attribute from BambooHR to ExtensionAttribute1 in AAD
                Write-PSLog -Message "Executing: Update-MgUser -UserId $aadObjectID -OnPremisesExtensionAttributes @{extensionAttribute1 = $bhrlastChanged } " -Severity Debug
            ...
            if ($aadLastName -ne $bhrLastName) {
              Write-PSLog -Message " Last name in AAD $aadLastName does not match in BHR $bhrLastName" -Severity Debug
              Write-PSLog -Message " Changing the last name of $bhrWorkEmail from $aadLastName to $bhrLastName." -Severity Debug
              if ($TestOnly.IsPresent) {
                Write-PSLog -Message "Executing: Update-MgUser -UserId $aadObjectID -Surname $bhrLastName" -Severity Test
              }
              else {
                Write-PSLog -Message "Executing: Update-MgUser -UserId $aadObjectID -Surname $bhrLastName" -Severity Debug
                ...
                if (!$?) {
                  ...
                  Write-PSLog -Message " Successfully changed the last name of $bhrWorkEmail from $aadLastName to $bhrLastName." -Severity Information
                }
              }
            }
            ...
            if ($aadfirstName -ne $bhrfirstName) {
              Write-PSLog "AAD first name '$aadfirstName' is not equal to BHR first name '$bhrFirstName'" -Severity Debug
              if ($TestOnly.IsPresent) {
                Write-PSLog -Message "Executing: Update-MgUser -UserId $aadObjectID -GivenName $bhrFirstName" -Severity Test
              }
              else {
                Write-PSLog -Message "Executing: Update-MgUser -UserId $aadObjectID -GivenName $bhrFirstName" -Severity Debug
                Update-MgUser -UserId $aadObjectID -GivenName $bhrFirstName
                if (!$?) {

                  Write-PSLog -Message "Could not change the First Name of $aadObjectID. Error details below. `n`nException: $($Error.exception) `nTarget object: $($error.TargetObject) `nDetails: $($error.ErrorDetails) `nStackTrace: $($error.ScriptStackTrace)" -Severity Error
                  $error.Clear()
                }
                else {
                  Write-PSLog -Message " Successfully changed $aadObjectID first name from $aadFirstName to $bhrFirstName." -Severity Information
                }
              }
            }
            ...
            if ($aadDisplayname -ne $bhrDisplayName) {
              Write-PSLog -Message "AAD Display Name $aadDisplayname is not equal to BHR $bhrDisplayName" -Severity Debug
              if ($TestOnly.IsPresent) {
                Write-PSLog -Message "Executing: Update-MgUser -UserId $aadObjectID -DisplayName $bhrdisplayName" -Severity Test
              }
              else {
                Write-PSLog -Message "Executing: Update-MgUser -UserId $aadObjectID -DisplayName $bhrdisplayName" -Severity Debug
                ...
                if (!$?) {
                  ...
                  Write-PSLog " Display name $aadDisplayName of $aadObjectID changed to $bhrDisplayName." -Severity Information
                }
              }
            }
            ...
            if ($aadWorkEmail -ne $bhrWorkEmail) {
              Write-PSLog -Message "AAD work email $aadWorkEmail does not match BHR work email $bhrWorkEmail"
              if ($TestOnly.IsPresent) {
                Write-PSLog -Message "Executing: Update-MgUser -UserId $aadObjectID -Mail $bhrWorkEmail"
              }
              else {
                Write-PSLog -Message "Executing: Update-MgUser -UserId $aadObjectID -Mail $bhrWorkEmail"
                ...
                if (!$?) {
                  ...
                  Write-PSLog "The current Email Address: $aadworkemail of $aadObjectID has been changed to $bhrWorkEmail." -Severity Warning
                }
              }
            }
            ...
            if ($aadUpn -ne $bhrWorkEmail) {
              Write-PSLog -Message "aadUPN $aadUpn does not match bhrWorkEmail $bhrWorkEmail" -Severity Debug
              if ($TestOnly.IsPresent) {
                Write-PSLog -Message "Executing: Update-MgUser -UserId $aadObjectID -UserPrincipalName $bhrWorkEmail" -Severity Test
              }
              else {
                Write-PSLog -Message "Executing: Update-MgUser -UserId $aadObjectID -UserPrincipalName $bhrWorkEmail" -Severity Debug
                Update-MgUser -UserId $aadObjectID -UserPrincipalName $bhrWorkEmail

                if (!$?) {

                  Write-PSLog -Message " Error changing UPN for $aadObjectID. `n Exception: $($Error.exception) `nTarget object: $($error.TargetObject) `nDetails: $($error.ErrorDetails) `nStackTrace: $($error.ScriptStackTrace)" -Severity Error
                  $error.Clear()
                }
                else {
                  Write-PSLog -Message " Changed the current UPN:$aadUPN of $aadObjectID to $bhrWorkEmail." -Severity Warning
                  $params = @{
                    Message         = @{
...
<p>Your email address was changed in the $CompanyName BambooHR. Your user account has been changed accordingly.</p><ui><li>Use your new user name: $bhrWorkEmail</li><li>Your password has not been modified.</li></ul><br/><p>$EmailSignature</p>"
          ...
          if ($aadUpnObjDetails.Capacity -eq 0 -and $aadEidObjDetails.Capacity -eq 0 -and ($bhrAccountEnabled -eq $true)) {
            ...
            if ($TestOnly.IsPresent) {
              # Write logging here
              Write-PSLog -Message "Executing New-MgUser -EmployeeId $bhremployeeNumber -Department $bhrDepartment -CompanyName $CompanyName -Surname $bhrlastName -GivenName $bhrfirstName -DisplayName $bhrdisplayName -AccountEnabled -Mail $bhrWorkEmail -OfficeLocation $bhrOfficeLocation `
              ...
              Write-PSLog -Message "$bhrWorkEmail does not have an AAD account and hire date ($bhrHireDate) is less than $DaysAhead days from now." -Severity Information
              ...
              $user = New-MgUser -EmployeeId $bhrEmployeeNumber -Department $bhrDepartment -CompanyName $CompanyName -Surname $bhrlastName -GivenName $bhrfirstName -DisplayName $bhrdisplayName `
              ...
              if ($null -eq $user) {
                ...
                Write-PSLog -Message "Account $bhrWorkEmail creation failed. New-Mguser cmdlet returned error. `n $($error | Select-Object *)"-Severity Error
                $params = @{
                  Message         = @{
                ...
                if ([string]::IsNullOrWhiteSpace($bhrSupervisorEmail) -eq $false) {
                  ...
                  # Connect-ExchangeOnline -CertificateThumbprint $AADCertificateThumbprint -AppId $ExchangeClientAppId
                  # -Organization $TenantId -ShowBanner:$false | Out-Null
                  Connect-ExchangeOnline -ManagedIdentity -Organization $TenantId -ShowBanner:$false | Out-Null
          ...
          if ($bhrAccountEnabled) {
            Write-PSLog -Message "$bhrWorkEmail's hire date ($bhrHireDate) is more than $DaysAhead days from now." -Severity Information
          }
          else {
...
if (($TestOnly.IsPresent -eq $false ) -and ([string]::IsNullOrWhiteSpace($Script:logContent)) -eq $false) {
  ...
  Write-PSLog -Message "`n Completed sync at $(Get-Date) and ran for $($runtime.Totalseconds) seconds" -Severity Information
  ...
  Write-PSLog 'No log content to share, no message sent' -Severity Debug
